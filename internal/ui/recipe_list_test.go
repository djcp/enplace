...
func TestTypingKey_Tab_CyclesFocus(t *testing.T) {
	...
	if m6.filterFocus != ffMinRating {
		t.Errorf("tab from ffIsBread: want ffMinRating, got %d", m6.filterFocus)
	}

	m7 := updateList(m6, keySpecial(tea.KeyTab))
	if m7.filterFocus != ffSearch {
		t.Errorf("tab from ffMinRating: want ffSearch, got %d", m7.filterFocus)
	}

	m8 := updateList(m7, keySpecial(tea.KeyTab))
	if m8.filterFocus != ffText {
		t.Errorf("tab from ffSearch should wrap to ffText, got %d", m8.filterFocus)
	}
}
